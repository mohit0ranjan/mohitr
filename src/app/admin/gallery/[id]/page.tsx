...
export default async function GalleryEditorPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (!isNew) {
        img = await prisma.galleryImage.findUnique({ where: { id } })
        ...
        <div className="space-y-6 max-w-xl mx-auto animate-in fade-in duration-500">
            ...
            <form action={saveImage} className="space-y-6">
                ...
                <div className="space-y-2">
                    ...
                    <input name="url" type="url" defaultValue={img?.url} required placeholder="https://..." className="w-full px-3 py-2 bg-neutral-900 border border-neutral-800 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all font-mono text-sm" />
                ...
                <div className="space-y-2">
                    ...
                    <textarea name="caption" defaultValue={img?.caption || ""} required rows={3} className="w-full px-3 py-2 bg-neutral-900 border border-neutral-800 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all" />
                </div>

                <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                        <label className="block text-sm font-medium text-neutral-300">Category</label>
                        <select name="category" defaultValue={img?.category || "Personal"} className="w-full px-3 py-2 bg-neutral-900 border border-neutral-800 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all">
